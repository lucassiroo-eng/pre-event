...
function IndustryPill({ value }: { value: string }) {
  return (
    <span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset", industryColorClass(value))}>
      {value}
    </span>
  );
}
...
export function RegionDetail({ code, deals, allDeals, onClose, onGenerateSlide }: Props) {
  const [openDialog, setOpenDialog] = useState<null | "clients" | "industries">(null);
  ...
  const industriesWithModules = useMemo(() => {
    ...
    return sorted.map(({ industry, count, biggest }) => {
      const modCounts = new Map<string, number>();
      for (const d of allDeals) {
        if (groupIndustry(d.sector) !== industry) continue;
        const mod = simplifyPlan(d.planName);
        if (mod) modCounts.set(mod, (modCounts.get(mod) ?? 0) + 1);
      }
      const topMod = Array.from(modCounts.entries()).sort((a, b) => b[1] - a[1])[0]?.[0] ?? "—";
      return { industry, count, biggest, topMod };
    });
  }, [deals, allDeals]);
    ...
    <div className="flex h-full w-full flex-col bg-card">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-muted/20 px-6 py-5 space-y-4">
        <Block
          ...
          expandLabel={`See all ${clientsByMrr.length}`}
          onExpand={() => setOpenDialog("clients")}
        >
        ...
        <Block
          ...
          expandLabel={`See all ${industriesWithModules.length}`}
          onExpand={() => setOpenDialog("industries")}
        >
          <IndustriesTable rows={industriesWithModules.slice(0, 5)} />
        </Block>
      </div>
      ...
      <Dialog open={openDialog !== null} onOpenChange={(o) => !o && setOpenDialog(null)}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>
              {openDialog === "clients" && `All clients · ${REGION_NAME[code] ?? code}`}
              {openDialog === "industries" && `All industries · ${REGION_NAME[code] ?? code}`}
            </DialogTitle>
          </DialogHeader>
          <div className="max-h-[70vh] overflow-y-auto">
            {openDialog === "clients" && <ClientsTable deals={clientsByMrr} />}
            {openDialog === "industries" && <IndustriesTable rows={industriesWithModules} />}
          </div>
        </DialogContent>
      </Dialog>
    </div>
...
function Block({
  icon, title, accent, iconBg, expandLabel, onExpand, children,
}: {
  icon: React.ReactNode; title: string; accent: string; iconBg: string;
  expandLabel: string; onExpand: () => void; children: React.ReactNode;
}) {
  return (
    <section className="relative overflow-hidden rounded-xl border border-border bg-card shadow-sm">
      <div className={cn("pointer-events-none absolute inset-x-0 top-0 h-16 bg-gradient-to-b", accent)} />
      <header className="relative flex items-center justify-between gap-3 px-4 pt-4 pb-3">
        ...
        >
          {expandLabel} <Maximize2 className="h-3 w-3 ml-1" />
        </button>
      </header>
...
function IndustriesTable({ rows }: { rows: { industry: string; count: number; biggest: WonDeal | null; topMod: string }[] }) {
      ...
      <table className="w-full text-sm">
        <thead className="bg-muted/50 text-[10px] uppercase tracking-wider text-muted-foreground">
          <tr>
            ...
            <th className="px-3 py-2 text-left font-medium">Módulo top · país</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => (
            <tr key={row.industry} className="border-t border-border hover:bg-muted/30">
              <td className="px-3 py-2">
                <IndustryPill value={row.industry} />
              </td>
              <td className="px-3 py-2 text-right tabular-nums font-semibold">{row.count}</td>
              <td className="px-3 py-2 text-xs text-muted-foreground">
                {row.topMod !== "—" ? (
                  <span className="rounded-md bg-muted px-2 py-0.5 font-medium text-foreground">{row.topMod}</span>
                ) : "—"}
              </td>
            </tr>
          ))}
